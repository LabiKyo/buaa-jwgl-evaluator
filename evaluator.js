...
function evaluate() {
  ...
  try { // POST的时候运行函数会导致异常
    ...
    iDoc = iframe.contentWindow || iframe.contentDocument; // iDoc是iframe中的document
    if (iDoc.document) {
      iDoc = iDoc.document;
    }

    trs = iDoc.getElementById('divJs').getElementsByTagName('tr'); // 获取所有tr
    j = 1;
    selects = trs[j].getElementsByTagName('select'); // 获取第一行的所有select,因为有些课程不止一个教师
    if (selects.length) {
      fillTr(selects, 2);
    } else {
      ++j;
